		...
		for (dirpath, dirnames, filenames) in os.walk(directory):
			...
			for i in range(0,len(files)):
				...
				if filename not in onlyfiles:
					...
					decrypt_file(username,temp)
					#ADD DECRYPT HERE	
					print("File "+temp + " downloaded")
					...
					if hash_client==hash_web:
						...
						print("File " + temp + " is on both Linux Client and Web server with different content.")
						print("Enter 1 to overwrite the file on the web server from the linux client")
						print("Enter 2 to overwrite the file on the linux client from the web server")
						...
						if(num=="2"):
							...
							with open(temp,'wb') as t:
								t.write(p2.content)
							decrypt_file(username,temp)
							print("File " + temp +" on linux client overwritten")
							#ADD DECRYPT HERE
						elif(num=="1"):
							...
							for i in range(0,len(fls)):
								...
								if fn==filename:
									...
									with open(temp,'wb+') as f:
										f.write(x)
									print("File " + temp +" on webserver overwritten")
									#file is deleted. Now upload
